    ...
    for match in matches:
        ...
        for line in lines:
            ...
            if line_stripped and not line_stripped.startswith('#'):
                ...
                if quote_match:
                    translation_content = f'"{quote_match.group(1)}"'
                    break
        ...
        if translate_match:
            ...
            while i < len(lines):
                ...
                if current_stripped == '""' or (current_stripped.startswith('"') and current_stripped.endswith('"') and len(current_stripped) <= 3):
                    ...
                    # 检查是否包含空引号（如 Toord ""）
                    if re.search(r'(\w+)\s+""', current_stripped):
                        if identifier in original_translations:
                            # 提取前面的角色名和引号部分
                            indent_match = re.match(r'^(\s*)', current_line)
                            indent = indent_match.group(1) if indent_match else ""
                            ...
                            # 提取角色名
                            char_match = re.search(r'(\w+)\s+""', current_stripped)
                            if char_match:
                                char_name = char_match.group(1)
                                result_lines.append(f'{indent}{char_name} {original_translations[identifier]}')
                                print(f"匹配并替换: {identifier} -> {original_translations[identifier]}")
                                replacement_count += 1
                            else:
                                result_lines.append(current_line)
