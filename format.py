...
def extract_original_translations_by_id(content):
    ...
    for match in matches:
        ...
        if translation_content:
            ...
            #临时措施print(f"提取到原文翻译: {identifier} -> {translation_content}")
        # 添加对"..."的临时支持 - 开发者可以随时删除此部分
        elif re.search(r'"(\.{3}|…)"', block_content):
            # 如果原文包含"..."或"…", 临时保留这些内容
            # 使用者可以随时删除下面这段代码来移除此功能
            dot_match = re.search(r'"(\.{3}|…)"', block_content)
            if dot_match:
                dots_text = dot_match.group(0)
                translations[identifier] = dots_text
                #临时措施print(f"临时处理'...': {identifier} -> {dots_text} (此为临时支持，可随时删除)")
    ...
    for match in matches2:
        ...
        # 添加对"..."的临时支持 - 开发者可以随时删除此部分
        elif identifier not in translations and re.search(r'"(\.{3}|…)"', block_content):
            # 如果原文包含"..."或"…", 临时保留这些内容
            # 使用者可以随时删除下面这段代码来移除此功能
            dot_match = re.search(r'"(\.{3}|…)"', block_content)
            if dot_match:
            ...
            while i < len(lines):
                ...
                if current_stripped == '""' or (current_stripped.startswith('"') and current_stripped.endswith('"') and len(current_stripped) <= 3):
                    ...
                    if identifier in original_translations:
                        ...
                        #临时措施print(f"匹配并替换: {identifier} -> {original_translations[identifier]}")
                        replacement_count += 1
                    else:
                        # 没有找到匹配的翻译，记录标识符
